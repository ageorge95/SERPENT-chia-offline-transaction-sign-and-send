...
initial_config = {'AEC': {'root': '{userdir}\\.aedge\\mainnet'.format(userdir=os_path.expanduser("~")),
                 ...
                 'SIT': {'root': '{userdir}\\.sit\\mainnet'.format(userdir=os_path.expanduser("~")),
                         ...
                         'friendly_name': 'silicoin',
                          ...
                          'module_to_use': 'silicoin_blockchain'},
...
class SERPENT_back_end():
    ...
    def check_overwrite_imports(self,
                          coin):
        if coin in ['SIT']:
            to_exec = 'from {module}.consensus.cost_calculator import calculate_cost_of_program\n\
                        from {module}.consensus.default_constants import DEFAULT_CONSTANTS\n\
                        from {module}.full_node.bundle_tools import simple_solution_generator\n\
                        from {module}.full_node.mempool_check_conditions import get_name_puzzle_conditions\n\
                        from {module}.rpc.full_node_rpc_client import FullNodeRpcClient\n\
                        from {module}.types.announcement import Announcement\n\
                        from {module}.types.blockchain_format.coin import Coin\n\
                        from {module}.types.blockchain_format.program import Program, SerializedProgram\n\
                        from {module}.types.coin_record import CoinRecord\n\
                        from {module}.types.coin_solution import CoinSolution\n\
                        from {module}.types.condition_opcodes import ConditionOpcode\n\
                        from {module}.types.blockchain_format.sized_bytes import bytes32\n\
                        from {module}.types.spend_bundle import SpendBundle\n\
                        from {module}.util.bech32m import encode_puzzle_hash, decode_puzzle_hash\n\
                        from {module}.util.condition_tools import parse_sexp_to_conditions\n\
                        from {module}.util.config import load_config\n\
                        from {module}.util.ints import uint16, uint64\n\
                        from {module}.util.hash import std_hash\n\
                        from {module}.util.condition_tools import conditions_dict_for_solution, pkm_pairs_for_conditions_dict\n\
                        from {module}.util.keychain import mnemonic_to_seed\n\
                        from {module}.wallet.wallet import Wallet\n\
                        from {module}.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (\
                            puzzle_for_pk,\
                            calculate_synthetic_secret_key,\
                            DEFAULT_HIDDEN_PUZZLE_HASH,\
                        )'
            exec(to_exec.format(module=self.config[coin]['module_to_use']))
    ...
    async def initiate_transfer(self,
        ...
        if self.check_mnemonic_integrity(mnemonic=mnemonic):
            ...
            try:
                self.check_overwrite_imports(coin=coin)
